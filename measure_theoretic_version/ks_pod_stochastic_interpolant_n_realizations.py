...
The script:
1. optionally selects any requested number of realizations;
2. splits entire selected realizations into train/validation/test sets;
3. fits the POD mean and basis only on training realizations;
4. places four uniformly spaced point sensors on the periodic domain;
5. constructs non-overlapping delay-coordinate windows within each realization;
6. saves a dataset suitable for the conditional stochastic-interpolant pipeline;
7. optionally trains the Gaussian-source stochastic interpolant;
8. visualizes trajectories, sensors, POD spectra, coefficient reconstruction,
...
def select_realizations(
    ...
    """
    Select an arbitrary number of realizations from an array of shape (N,T,D).

    Returns
    -------
    selected_trajectories:
        The selected realization subset.
    original_indices:
        Indices of the selected realizations in the original input file.

    Notes
    -----
    At least three realizations are required because the downstream code creates
    nonempty train, validation, and test splits by entire realization.
    """
    n_available = trajectories.shape[0]

    if n_realizations is None:
        selected_indices = np.arange(n_available, dtype=np.int64)
    else:
        if n_realizations < 3:
            raise ValueError(
                "--n-realizations must be at least 3 so train, validation, "
                "and test realization splits can all be nonempty."
            )
        if n_realizations > n_available:
            raise ValueError(
                f"Requested {n_realizations} realizations, but the input file "
                f"contains only {n_available}."
            )

        if selection == "first":
            selected_indices = np.arange(n_realizations, dtype=np.int64)
    ...
    if selected_indices.size < 3:
        raise ValueError(
            "At least three realizations are required for train/validation/test."
        )

    return trajectories[selected_indices], selected_indices
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=100,
        help=(
            "Number of realizations to use from the input array. "
            "Default: use all available realizations."
        ),
    )
    p.add_argument(
        ...
        default="first",
        help=(
            "How to choose realizations when --n-realizations is smaller than N. "
            "'first' uses indices 0,...,n-1; 'random' samples without replacement "
            "using --selection-seed."
        ),
    )
    p.add_argument(
        "--selection-seed",
        type=int,
        default=17,
        help="Random seed used only when --realization-selection=random.",
    )
...
def main() -> None:
    ...
    trajectories, selected_original_indices = select_realizations(
        ...
        selection=args.realization_selection,
    ...
    metadata = {
        ...
        "selected_original_realization_indices": selected_original_indices.tolist(),
