...
plot:
    Reuse an already saved ensemble NPZ and redraw the UQ plot without either
...
@dataclass(frozen=True)
class PlotConfig:
    """Appearance settings for the single all-sensor UQ figure."""
...
def choose_uniform_sensor_layout(
    ...
    *,
    ...
    """
    Place sensors uniformly on a periodic Cartesian grid.

    The one-dimensional index sets are

        floor(k*n_x/sensors_x), k=0,...,sensors_x-1,
        floor(l*n_y/sensors_y), l=0,...,sensors_y-1.
...
def plot_all_sensor_forecasts(
    ...
    *,
    ...
    """
    Create one axes containing every sensor forecast, interval, and truth.
    """
    ...
    if np.unique(trajectories).size != 1:
        raise ValueError(
            "The single anchor-time plot requires conditions from "
            "one trajectory."
        )
    ...
    for sensor_index in range(sensors.num_sensors):
        label_base = (
            f"S{sensor_index + 1} "
            f"({sensors.x_positions[sensor_index]:.3g}, "
            f"{sensors.y_positions[sensor_index]:.3g})"
        )
        ...
        axis.plot(
            ...
            linewidth=plot_config.line_width,
            linestyle="--",
            label=f"{label_base} truth",
            zorder=4,
        )

    trajectory_index = int(trajectories[0])
    axis.set_xlabel("Anchor time index t")
    axis.set_ylabel("Forecasted sensor value")
    axis.set_title(
        f"{plot_config.title}\n"
...
def create_uq_visualization(
    ...
    *,
    ...
    """Create the all-sensor plot and save its numerical plotting data."""
