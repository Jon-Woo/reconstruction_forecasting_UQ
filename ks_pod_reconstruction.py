...
13. Optionally reuse a saved dataset and trained checkpoint in inference-only
    mode, generating new conditional ensembles and UQ plots without retraining.
...
Execution modes
---------------
Training mode prepares the data, trains the drift, samples conditional
ensembles, and creates the UQ plots.
...
from a previous training run, generates new Euler--Maruyama ensembles, and
recreates the UQ plots without calling train_conditional_drift.
...
def load_prepared_test_data_from_npz(
    ...
    """
    Reconstruct the test-data structure needed for inference-only sampling.

    The NPZ file must have been produced by this pipeline and must contain the
    normalized test conditions/targets, physical-coordinate copies, and the
    trajectory/anchor/window provenance arrays.

    Returns
    -------
    prepared:
        Minimal dictionary accepted by generate_test_ensembles.
    metadata:
        Dataset dimensions and saved preprocessing information used for
        validation and the inference summary.
    """
    ...
    with np.load(dataset_path, allow_pickle=False) as data:
        ...
        if missing:
            raise KeyError(
                "The prepared dataset is missing required inference arrays: "
                + ", ".join(missing)
            )
    ...
    if normalized_y.ndim != 2 or normalized_u.ndim != 2:
        raise ValueError(
            "Y_test and U_test must both be two-dimensional arrays."
        )

    metadata = {
        "dataset_path": str(dataset_path),
        "num_test_pairs": int(normalized_y.shape[0]),
        "condition_dimension": int(normalized_y.shape[1]),
        "target_dimension": int(normalized_u.shape[1]),
        **normalization_metadata,
        "saved_metadata": metadata_json,
    }

    arrays_with_pair_axis = {
        "U_test": normalized_u,
        "Y_test_physical": physical_y,
        "U_test_physical": physical_u,
        "trajectory_index_test": trajectory_index,
        "anchor_index_test": anchor_index,
        "window_start_test": window_start,
        "window_end_test": window_end,
    }

    num_pairs = normalized_y.shape[0]

    for name, values in arrays_with_pair_axis.items():
        if values.shape[0] != num_pairs:
            raise ValueError(
                f"{name} has {values.shape[0]} pairs, but Y_test has "
                f"{num_pairs}."
            )
...
def plot_mode_reconstruction_intervals(
    ensembles: Mapping[str, Any],
    *,
    ...
    """
    Create one reconstruction plot for each retained POD mode.

    For mode j, the plot shows, at every selected test anchor time:

        * the conditional ensemble-mean estimate of a_j(t);
        * the componentwise central empirical confidence interval; and
        * the true POD coefficient a_j(t).

    When all selected conditions come from one trajectory, the horizontal
    coordinate is the actual anchor-time index. When several trajectories are
    represented, the horizontal coordinate is the ordered test-condition
    number and each tick is labeled ``trajectory:anchor``.
    """
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    samples = np.asarray(ensembles["samples"])
    truths = np.asarray(ensembles["true_coefficients"])
    ...
    if samples.ndim != 3:
        raise ValueError(
            "ensembles['samples'] must have shape "
            "(num_conditions, ensemble_size, num_modes)."
        )
    if truths.shape != (samples.shape[0], samples.shape[2]):
        raise ValueError(
            "true_coefficients must have shape "
            "(num_conditions, num_modes)."
        )
    if trajectory_indices.shape != (samples.shape[0],):
        raise ValueError(
            "trajectory_index must have one entry per condition."
        )
    if anchor_indices.shape != (samples.shape[0],):
        raise ValueError(
    ...
    # Order first by trajectory and then chronologically by anchor time.
    order = np.lexsort((anchor_indices, trajectory_indices))
    means = means[order]
    truths = truths[order]
    lower = lower[order]
    upper = upper[order]
    trajectory_indices = trajectory_indices[order]
    anchor_indices = anchor_indices[order]

    unique_trajectories = np.unique(trajectory_indices)
    one_trajectory = unique_trajectories.size == 1

    if one_trajectory:
        x_values = anchor_indices.astype(np.float64)
        x_label = "Anchor time index t"
        tick_labels = None
    else:
        x_values = np.arange(samples.shape[0], dtype=np.float64)
        x_label = "Selected test condition (trajectory:anchor)"
        tick_labels = [
            f"{trajectory}:{anchor}"
            for trajectory, anchor in zip(
                trajectory_indices,
                anchor_indices,
            )
        ]
    ...
    for mode_index in range(samples.shape[2]):
        mode_number = mode_index + 1
        mode_mean = means[:, mode_index]
        mode_truth = truths[:, mode_index]
        mode_lower = lower[:, mode_index]
        mode_upper = upper[:, mode_index]

        figure, axis = plt.subplots(figsize=(11, 6))

        # Shade the empirical central interval across anchor times.
        axis.fill_between(
            x_values,
            mode_lower,
            mode_upper,
        ...
        axis.plot(
            ...
            marker="x",
            linestyle="--",
            label="True POD coefficient",
        )

        axis.set_xlabel(x_label)
        axis.set_ylabel(f"POD coefficient a_{mode_number}(t)")

        if one_trajectory:
            trajectory_text = (
                f"trajectory={int(unique_trajectories[0])}"
            )
        else:
            trajectory_text = (
                f"{unique_trajectories.size} trajectories represented"
            )

        axis.set_title(
            f"Conditional reconstruction of POD mode {mode_number}\n"
            f"{trajectory_text}; interval level={interval_level:.0%}"
        )
        ...
        output_path = output_dir / (
            f"pod_mode_{mode_number:03d}.png"
        )
        figure.savefig(
            output_path,
            dpi=200,
...
def create_uq_visualizations(
    ...
    *,
    ...
    mode_paths = plot_mode_reconstruction_intervals(
        ensembles,
        interval_level=uq_config.interval_level,
        output_dir=output_dir / "modes",
    )
...
def run_inference_only(
    ...
    *,
    ...
    """
    Generate new POD-coefficient ensembles and UQ plots without retraining.

    This function does not load the original trajectory array, refit POD,
    rebuild the delay pairs, or call train_conditional_drift. It reuses the
    normalized test conditions and physical POD targets saved in the prepared
    NPZ file, together with the trained drift checkpoint.

    The stochastic source and Euler--Maruyama increments are drawn again, so
    changing the UQ seed or ensemble size creates a new conditional ensemble
    while leaving the trained model fixed.
    """
    uq_config.validate()
    ...
    prepared, dataset_metadata = load_prepared_test_data_from_npz(
        dataset_path
    )
    ...
    checkpoint_model_config = ensembles["checkpoint"].get(
        "model_config",
        {},
    )
    checkpoint_condition_dim = int(
        checkpoint_model_config.get(
            "condition_dim",
            dataset_metadata["condition_dimension"],
        )
    )
    checkpoint_target_dim = int(
        checkpoint_model_config.get(
            "target_dim",
    ...
    if (
        ...
        raise ValueError(
            ...
            f"{dataset_metadata['condition_dimension']}."
        )

    if checkpoint_target_dim != dataset_metadata["target_dimension"]:
        raise ValueError(
            "Checkpoint/data target-dimension mismatch: checkpoint expects "
            f"{checkpoint_target_dim}, while the dataset has "
            f"{dataset_metadata['target_dimension']}."
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train a stochastic-KS POD reconstruction model, or reuse a "
            "saved checkpoint in inference-only mode to generate new "
            "conditional ensembles and UQ plots without retraining."
        )
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("ks_pod_train_uq_output"),
        help=(
            "Training output directory. In inference mode this is also used "
            "to locate default dataset/checkpoint paths."
        ),
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Prepared ks_pod_delay_data.npz used in inference mode. "
            "Defaults to OUTPUT_DIR/ks_pod_delay_data.npz."
        ),
    )
    parser.add_argument(
        "--checkpoint-path",
        ...
        default=None,
        help=(
            "Trained checkpoint used in inference mode. Defaults to "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Delay in stored time indices; required in training mode.",
    )
    parser.add_argument(
        "--gap",
        type=int,
        default=None,
        help="Gap between delay windows; required in training mode.",
    )
    parser.add_argument(
        "--energy-threshold",
        type=float,
...
def main() -> None:
    ...
    if args.mode == "inference":
        dataset_path = (
            ...
            else args.output_dir / "ks_pod_delay_data.npz"
        )
        checkpoint_path = (
